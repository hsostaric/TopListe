...
if(!isset($_GET['oglas_id'])){
    header("Location:GalerijaMojihOglasa.php");
}
$mod=$_GET['oglas_id'];
?>
...
<html>
    ...
    <body>
        ...
        <section> 
            ...
            <div class="pozicija2">
                <h2>Obrazac promjene za oglas</h2>
                <form novalidate id = 'prijava' enctype="multipart/form-data" method = 'post' name = 'form1' action ='<?php echo $_SERVER['PHP_SELF']."?oglas_id=".$_GET['oglas_id']?>'>
                    <?php
                    $vez = new Baza();
                    $vez->spojiDB();
                    $id=$_SESSION['id'];
                    $upit="Select *from oglas where id_zahtjev_za_oglas='".$mod."' and korisnik_id_korisnik='".$id."'";
                    $rez=$vez->selectDB($upit);
                    $red=$rez->fetch_assoc();
                    $vez->zatvoriDB();
                    ?>
                    <div class="row">
                    ...
                    <div class="row">
                        ...
                        <div class="col-75">
                            <input class="textboxovi" type="text" id="nazivoglasa" name="nazivoglasa"  placeholder="Unesite naziv oglasa" value="<?php echo $red['naziv'] ?>">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class='col-75'>
                            <textarea class="odredjeno"  rows="40" cols="60" id="nazivoglasa" name="opisoglasa"  placeholder="Unesite opis oglasa" value="<?php echo $red['opis'] ?>" ></textarea>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-75">
                            <input class="textboxovi" type="text" id="urloglasa" name="urlOglasa"  placeholder="Unesite naziv oglasa" value="<?php echo $red['url'] ?>">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <input name="urediOglas" type="submit" value="Uredi oglas" id="potvrda">
                    </div>
                    <?php
                    if (isset($_POST['urediOglas'])) {
                        ...
                        $radnja = "Ažuriran je oglas";
                        $datum = dohvatiVirtualnoVrijeme();
                        $kor = $_SESSION["korime"];
                        $dnevnik = "Insert into `dnevnik_rada` values('default','$kor','$datum','$radnja')";
                        $veza->updateDB($dnevnik);

                        $sql = "Update oglas set naziv='".$naziv."',vrsta_oglasa_id='".$vrstaOglasa."',opis='".$opisOglasa."',url='".$urlOglasa."',pocetak_prikazivanja='".$pocetak."',zavrsetak_prikazivanja='".$kraj."',slika='".$slika."' where id_zahtjev_za_oglas='".$mod."'";
                        $veza->updateDB($sql);
                        $veza->zatvoriDB();
